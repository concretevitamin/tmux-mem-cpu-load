...
void print_help() {
  ...
  cout << "tmux-mem-cpu-load v" << tmux_mem_cpu_load_VERSION << endl
       ...
       << "-p, --powerline-left\n"
       << "\tUse powerline left symbols throughout the output, enables "
          "--colors\n"
       << "-q, --powerline-right\n"
       << "\tUse powerline right symbols throughout the output, enables "
          "--colors\n"
       ...
       << "\tSet memory display mode. 0: Default, 1: Free memory, 2: Usage "
          "percent.\n"
       << "-t <value>, --cpu-mode <value>\n"
       << "\tSet cpu % display mode. 0: Default max 100%, 1: Max 100% * number "
          "of threads. \n"
...
int main(int argc, char **argv) {
  ...
  int c;
  // while c != -1
  while ((c = getopt_long(argc, argv, "hi:cpqg:m:a:t:", long_options, NULL)) !=
         -1) {
    switch (c) {
    case 'h': // --help, -h
      print_help();
  ...
  if (argc > optind) {
    std::cout << "The interval and graph lines options are now specified with "
                 "flags.\n\n";
    print_help();
    return EXIT_FAILURE;
  }
  ...
  std::cout << std::put_time(&tm, " %a %b %d %H:%M | ")
            << mem_string(memory_status, mem_mode, use_colors,
                          use_powerline_left, use_powerline_right)
            // << cpu_string( cpu_mode, cpu_usage_delay, graph_lines,
            // use_colors, use_powerline_left, use_powerline_right )
            << " ["
